...
Original file is located at
    https://colab.research.google.com/drive/19lZzGzorh8CS2CNFFw1XTRj29w0B-dMY
"""
...
mlp_history = mlp_model.fit(x_train, y_train, epochs=50, batch_size=128, validation_data=(x_test, y_test))
end_time = time.time()
...
#trainig cnn model at 50 epoch
import time 
start_time = time.time()
cnn_history = cnn_model.fit(x_train, y_train, epochs=50, batch_size=128, validation_data=(x_test, y_test))
end_time = time.time()
...
start_time= time.time()
lenet_history = lenet_model.fit(x_train, y_train, epochs=50, batch_size=128, validation_data=(x_test, y_test))
end_time = time.time()
...
start_time= time.time()
lenet5_history = lenet5_model.fit(x_train, y_train, batch_size=128, epochs=50,validation_data=(x_test, y_test))
end_time = time.time()
print("Time taken: {:.2f} seconds".format(end_time - start_time))
...
# ResNet training
start_time= time.time()
resnet_history = resnet.fit(x_train, y_train, epochs=50, batch_size=128, validation_data=(x_test, y_test))
end_time = time.time()
print("Time taken: {:.2f} seconds".format(end_time - start_time))
